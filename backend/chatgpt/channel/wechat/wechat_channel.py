...
class WechatChannel(Channel):
    ...
    def handle(self, msg):
        ...
        friend_name = msg['User'].get('RemarkName', None)
        content = msg['Text']
        print(content)
        # 任何人说话
        ...
        if from_user_id == other_user_id and friend_name in config.get(
                'friends_name_white_list') and match_prefix is not None:
            # 好友向自己发送消息
            if match_prefix and match_prefix != '':
    ...
    def _do_send(self, query, reply_user_id):
        try:
            ...
            if reply_text and 'ChatGPT' not in reply_text and 'AI' not in reply_text:
